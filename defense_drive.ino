...
//  boolean LClick = ps2x.Analog(PSB_PAD_LEFT); // This dictates the pivot point junk.
...
*/
  ...
  */
  if(ps2x.ButtonPressed(PSB_L3)){
    if(pivot == 0){
      pivot = 1; 
      Serial.println("Pivot mode initiated");
    }
     else{
      pivot = 0;
      Serial.println("Normal drive mode initiated");
     }
   Serial.println("Hello, I've been pressed, please don't touch me, stranger danger.");
  }
  double factor_straff = 0.55; //The correct value is inbetween 0.5 and 0.6: Tested so far: 0.5, 0.55
                               //closest to getting us straight: 0.55
  //Here is where we set each motor's speed and direction. 
  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  SPEED 1
  int speed1;
  if(pivot == 0) //IF the pivot mode is enabled, we'll...pivot around the def-square.
    speed1 = straight - (factor_straff*side) + turn; //but this is for when we're not doing def-mode.
  else 
    speed1= straight - side + turn;
  if(speed1 >= 0)
  {
  ...
  int speed3;
  if(pivot == 0) //IF the pivot mode is enabled, we'll...pivot around the def-square.
    speed3 = -straight - (factor_straff*side) + turn; //but this is for when we're not doing def-mode.
  else 
    speed3= -straight - side + turn;
